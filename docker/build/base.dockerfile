...
RUN apt-get install -y  libtool autoconf automake  libcppunit-dev maven
RUN apt-get install -y  libgoogle-glog-dev gdb
RUN apt-get install libgoogle-glog0v5
...
COPY install/zk /tmp/install/zk
RUN /tmp/install/zk/ZkClient.sh
